...
4. Offline CLI
   ...
   - `tools/kernel_corpus/` provides the consumer-side Kernel Corpus pack layer: SQLite builder, freshness validator, read-only query CLI, stdio MCP server, lifecycle tracer, subsystem atlas generator, answer harness, expanded lifecycle ontologies, lifecycle cross-topic candidate penalties, relevance-filtered atlas hubs, and the `kernel-corpus-analysis` skill
